...
def success_log(response, submission_uuid, status, invoice_number, company_doc=None):
    ...
    try:
        ...
        existing_doc = frappe.db.exists(
            SUCCESS_LOG_DOCTYPE, {"invoice_number": invoice_number}
        )
        if existing_doc:
            # Update the existing document
            doc_instance = frappe.get_doc(SUCCESS_LOG_DOCTYPE, existing_doc)
            doc_instance.update(
                {
            ...
            frappe.log(f"Created new LHDN's Log of success: {invoice_number}")
    ...
    try:
        ...
        if not submission_uid:
            if isinstance(sales_invoice_doc, dict):
                sales_invoice_doc = frappe.get_doc(SALES_DOCTYPE, invoice_number)
...
def status_submit_success_log(doc: typing.Union[str, typing.Dict[str, typing.Any]]):
    ...
    try:
        ...
        if response.status_code == 200:
            response_data = response.json()
            doc_instance = frappe.get_doc(SUCCESS_LOG_DOCTYPE, doc.get("name"))
            document_summary = response_data.get("documentSummary", [])
            if document_summary:
...
def build_invoice_base(invoice, sales_invoice_doc, company_abbr):
    """Build the base structure of the invoice with company and customer data."""
    ...
    if customer_doc.customer_name != GENERAL_PUBLIC:
    ...
    return invoice, customer_doc


def apply_tax_and_items(invoice, sales_invoice_doc, any_item_has_tax_template):
    """Apply tax and item data to the invoice based on whether any item has a tax template."""
...
def apply_signature_flow(company_abbr, line_xml, doc_hash):
    """defining the apply signature flow"""
    (
        certificate_base64,
        formatted_issuer_name,
        x509_serial_number,
